...
# this dict values and keys need to convert
old_dict = {
        # username
        # this must be converted because its gonna be converted somehow
...
# this list compherension makes everything great again
new_dict = dict([(value, key) for key, value in old_dict.items()]) 

newDefaultFile = replace_words(f, new_dict)

customConf = 'globFiles/convertedFiles/darkconf/kdeglobals'
with open(customConf, 'w') as glob:
    glob.write(newDefaultFile)
    glob.close()

# scheduler dosyalarındaki renk yerlerine direk olarak bu dosyaların çalıştırmaları komutunu ver
# os.system('cp -r globFiles/convertedFiles/darkconf ~/Masaüstü')
